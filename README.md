...
- Edges has two settings. Smooth, the default, splits every triangle the selection boundary crosses along the contour and paints only the pieces on the selected side, so a Height band ends on a straight line and a fill or brush region ends on the 0.5 iso-line of the per-corner selected-area fraction. Corners count only neighbors within 40° of the triangle's normal, so a boundary on a sharp crease stays exactly on the crease and splits nothing. Triangles keeps whole triangles. Recipes without the field replay as Triangles. On a 320k-triangle sphere a smooth Height paint takes about 125 ms and a smooth Fill about 280 ms, most of it building the per-vertex incidence table; the split changes the index array, so the viewport rebuilds its geometry and raycast tree on that paint. The split mesh stays manifold, and the extra triangles remain if the region is later repainted Base.
...
- In Fill mode the hover preview stays pinned when the pointer leaves the model, so the angle can be tuned from the panel and applied with Paint highlighted or nudged by 1° with [ and ].
